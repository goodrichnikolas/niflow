...
IMPORT_DEFAULTS_PATH = _NIFLOW_ROOT / "import_defaults.py"

_IMPORT_PROBE_GROUP = "niflow-import-probe"
_IMPORT_PROBE_BATCH = 40

_IMPORT_DEFAULTS_HEADER = '''"""AUTO-GENERATED by ``python -m niflow.codegen --import-defaults`` — do not edit.
...
def _import_probe_flow(name: str, proc_types, service_types):
    """A flow holding one instance of each given type, ready to push."""
    from niflow.core import ControllerService, Flow, Processor

    flow = Flow(name=name)
    for i, dt in enumerate(service_types):
        flow.add(ControllerService(name=f"svc{i}", type=dt.type))
    for i, dt in enumerate(proc_types):
        flow.add(Processor(name=f"proc{i}", type=dt.type,
                           position=(float(i % 20) * 400, float(i // 20) * 200)))
    return flow


def _probe_divergences(client: Any, group_id: str) -> dict:
    """``{type: {key: value}}`` for every property the import materialised.

    Only keys whose live value differs from the component's own descriptor
    default are recorded — everything else is already predictable offline, and
    a catalog is only worth what it costs to read.
    """
    out: dict = {}

    def record(type_str: str, properties: dict, descriptors: dict) -> None:
        for key, value in (properties or {}).items():
            if value is None:
                continue
            descriptor = (descriptors or {}).get(key) or {}
            if descriptor.get("identifiesControllerService"):
                continue  # a service reference is an instance id, not a default
            if descriptor.get("sensitive") or value == "********":
                continue  # NiFi masks sensitive values; a mask is not a default
            if value != descriptor.get("defaultValue"):
                out.setdefault(type_str, {})[key] = value
    ...
    for entity in services:
        ...
        record(component.get("type", ""), component.get("properties"),
               component.get("descriptors"))
    ...
    for entity in processors:
        ...
        record(component.get("type", ""), config.get("properties"),
               config.get("descriptors"))
    return out
...
def harvest_import_defaults(config: Optional[NiFiConfig] = None) -> Tuple[int, str]:
    ...
    Returns ``(types_with_divergence, nifi_version)`` and rewrites
    ``niflow/import_defaults.py`` with this line's block merged in — the other
    line's block is preserved, so the two servers can be harvested one at a
    time.

    Batched (and best-effort per batch) because a single type NiFi refuses to
    import would otherwise cost the whole sweep.
    """
    ...
    proc_named = [item for _, item in _unique_factory_names(procs)]
    svc_named = [item for _, item in _unique_factory_names(svcs)]
    logger.info(
        "Probing the import path on NiFi %s: %d processor + %d service type(s)",
        version, len(proc_named), len(svc_named),
    )
    ...
    batches = [
        ...
        ([], svc_named[i:i + _IMPORT_PROBE_BATCH])
        for i in range(0, len(svc_named), _IMPORT_PROBE_BATCH)
    ]
    for n, (proc_batch, svc_batch) in enumerate(batches, 1):
        name = f"{_IMPORT_PROBE_GROUP}-{n}"
        try:
        ...
        try:
            ...
            try:
                ...
                logger.error("Could not remove probe group %r: %s", name, exc)
...
def main() -> None:
    ...
    if "--dump-rulebook" in args:
        ...
        return
    if "--import-defaults" in args:
        n_types, version = harvest_import_defaults(NiFiConfig.from_env())
        print(
            f"Wrote {IMPORT_DEFAULTS_PATH.relative_to(_NIFLOW_ROOT.parent)}: "
            f"{n_types} type(s) whose import differs from their descriptors "
            f"(NiFi {version})"
        )
        return
    if "--compat" in args:
